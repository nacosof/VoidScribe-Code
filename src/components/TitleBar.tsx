...
export function TitleBar({ lang, recentWorkspaces = [], fileMenuItems = [], onOpenSettings, settingsActive = false, minimal = false, sidebarOpen = true, onToggleSidebar, chatOpen = true, onToggleChat, terminalOpen = true, onToggleTerminal, onRequestClose, }: TitleBarProps) {
    ...
    return (<header className="titlebar">
      <div className="titlebar__brand">
        ...
        {fileMenuItems.length && !minimal ? (<TitleBarFileMenu lang={lang} recentWorkspaces={recentWorkspaces} items={fileMenuItems}/>) : null}
      </div>
      <div className="titlebar__controls">
        {!minimal && onToggleSidebar ? (<button type="button" className={`titlebar__btn${sidebarOpen ? " titlebar__btn--active" : ""}`} onClick={onToggleSidebar} aria-label={sidebarOpen ? t(lang, "titleSidebarHide") : t(lang, "titleSidebarShow")} title={sidebarOpen ? t(lang, "titleSidebarHide") : t(lang, "titleSidebarShow")}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
              <rect x="3" y="4" width="18" height="16" rx="2"/>
              <path d="M9 4v16"/>
            </svg>
          </button>) : null}
        {!minimal && onToggleTerminal ? (<button type="button" className={`titlebar__btn${terminalOpen ? " titlebar__btn--active" : ""}`} onClick={onToggleTerminal} aria-label={terminalOpen ? t(lang, "titleTerminalHide") : t(lang, "titleTerminalShow")} title={t(lang, "titleTerminal")}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
              <rect x="3" y="4" width="18" height="14" rx="2"/>
              <path d="M7 8h6M7 12h10"/>
            </svg>
          </button>) : null}
        {!minimal && onToggleChat ? (<button type="button" className={`titlebar__btn${chatOpen ? " titlebar__btn--active" : ""}`} onClick={onToggleChat} aria-label={t(lang, "titleChat")} title={t(lang, "titleChat")}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
