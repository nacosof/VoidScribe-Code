...
def _append_cyrillic_identifier_errors(tree: ast.AST, errors: list[dict]) -> None:
    def check_name(name: str, lineno: int, col: int) -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Name):
            check_name(node.id, node.lineno, node.col_offset)
        elif isinstance(node, ast.FunctionDef):
            check_name(node.name, node.lineno, node.col_offset)
        elif isinstance(node, ast.AsyncFunctionDef):
            check_name(node.name, node.lineno, node.col_offset)
        elif isinstance(node, ast.ClassDef):
            check_name(node.name, node.lineno, node.col_offset)
        elif isinstance(node, ast.arg) and node.arg:
            check_name(node.arg, node.lineno, node.col_offset)
        elif isinstance(node, ast.Import):
            for alias in node.names:
                check_name(alias.asname or alias.name.split(".")[-1], node.lineno, node.col_offset)
        elif isinstance(node, ast.ImportFrom) and node.module:
            for alias in node.names:
                check_name(alias.asname or alias.name, node.lineno, node.col_offset)
...
def lint_file(file_path: str) -> list[dict]:
    ...
    _append_cyrillic_identifier_errors(tree, errors)
