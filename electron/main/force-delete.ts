...
import { WorkspaceError, isWorkspaceFsLockError, isWorkspaceFsMissingError, resolveWorkspacePath, } from "./workspace";
...
async function killProcessesLockingPath(workspaceRoot: string, targetFull: string): Promise<void> {
    ...
    const ps = [
        ...
        "Where-Object {",
        "  $_.Name -in @('node.exe','npm.exe','npx.exe') -and",
    ...
    try {
        await execFileAsync(resolveShellExecutable(), ["-NoProfile", "-Command", ps], { windowsHide: true, env: shellSpawnEnv(), timeout: 20000 });
    }
    catch {
    }
    await sleep(400);
}
async function shellForceRemove(fullPath: string, isDirectory: boolean): Promise<void> {
    ...
    if (process.platform === "win32") {
        ...
        await execFileAsync(resolveShellExecutable(), ["-NoProfile", "-Command", ps], { windowsHide: true, env: shellSpawnEnv(), timeout: 180000 });
        return;
    }
    await rm(fullPath, {
...
export async function forceDeleteWorkspaceEntry(workspaceRoot: string, targetPath: string): Promise<void> {
