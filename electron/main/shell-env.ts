...
export function shellSpawnEnoentMessage(): string {
    return ("Не удалось запустить shell (spawn ENOENT) — у процесса Electron нет powershell.exe в PATH. " +
        "Это не значит, что PowerShell или Node не установлены (терминал IDE может работать). " +
        "Используй write_file вместо run_command, или перезапусти VoidScribe из обычного терминала.");
}
