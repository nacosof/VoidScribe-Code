import { buildTruncationNudge } from "../agent-reliability";
import { AGENT_TOOL_NUDGE } from "../../../src/lib/agent-intent";
import { resolveAnthropicMaxTokens, resolveRequestTimeoutMs, type AiRuntimeSettings, } from "../ai";
...
import type { AgentSchedulerProvider, ChatTurn, SchedulerModelStep, } from "./provider-types";
...
function getAnthropicAgentTools(mode: AgentInteractionMode): AnthropicTool[] {
    return getAgentToolsForMode(mode)
        .filter((tool): tool is Extract<typeof tool, {
        type: "function";
    }> => tool.type === "function")
        .map((tool) => ({
...
export function createAnthropicSchedulerProvider(options: AnthropicSchedulerProviderOptions): AgentSchedulerProvider {
    const { settings, projectMature, interactionMode = "agent" } = options;
    ...
    return {
        ...
        async completeModelStep(input): Promise<SchedulerModelStep> {
            const response = await callAnthropic({
                ...
                signal: input.signal,
                tools: getAnthropicAgentTools(interactionMode),
            });
            lastAssistantContent = response.content;
            const toolUses = response.content.filter((block): block is AnthropicToolUseBlock => block.type === "tool_use");
            ...
            const toolCalls: AgentToolCallRequest[] = sortToolsForExecution(toolUses).map((toolUse) => ({
                id: toolUse.id,
                name: toolUse.name,
        ...
        recordToolBatchResults(results: AgentToolBatchResult[]) {
            const toolResults: AnthropicToolResultBlock[] = results.map(({ call, result }) => ({
                type: "tool_result",
                tool_use_id: call.id,
        ...
        recordNudgeUserMessage(step: SchedulerModelStep) {
            messages.push({
                role: "user",
                content: step.truncatedOutput ? buildTruncationNudge() : AGENT_TOOL_NUDGE,
            });
        },
        injectUserNudge(content: unknown) {
            messages.push({ role: "user", content: content as string | AnthropicContentBlock[] });
        },
    };
}
