...
import { resolveWorkspacePath, WorkspaceError, } from "./workspace";
...
async function walkFiles(workspaceRoot: string, dirPath: string, onFile: (relPath: string, fullPath: string) => Promise<void>): Promise<void> {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (IGNORE_DIR_NAMES.has(entry.name))
                continue;
            const nextDir = dirPath === "." ? entry.name : `${dirPath}/${entry.name}`.replace(/\\/g, "/");
...
export async function grepWorkspace(workspaceRoot: string, pattern: string, options?: {
    path?: string;
    glob?: string;
    maxResults?: number;
}): Promise<string> {
    ...
    await walkFiles(workspaceRoot, startPath, async (relPath, fullPath) => {
        if (truncated)
            return;
        if (!matchesGlob(relPath, options?.glob))
            return;
        let fileStat;
        try {
