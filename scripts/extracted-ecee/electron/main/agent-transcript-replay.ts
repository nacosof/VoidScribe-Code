...
export function transcriptTurnToOpenAiMessages(
...
export function chatHistoryToAgentOpenAiMessages(
  history: ChatMessage[],
  options?: ChatVisionOptions
): ChatCompletionMessageParam[] {
  const hasTranscript = history.some(
    (message) =>
      message.role === "assistant" && (message.agentTranscript?.length ?? 0) > 0
  );
  ...
  for (const message of history) {
    if (message.role === "user") {
      out.push(
        ...chatHistoryToOpenAiMessages([message], options).filter(
          (item) => item.role === "user"
        )
      );
      continue;
