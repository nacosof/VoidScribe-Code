...
import {
  AGENT_CHAT_RETRIES,
  AGENT_CHAT_RETRY_DELAY_MS,
  isRateLimitError,
  isToolCallGenerationError,
  rateLimitBackoffMs,
  sleep,
...
import {
  compactMessagesForApi,
  isLikelyContextOverflow400,
...
export function createOpenAiSchedulerProvider(
  options: OpenAiSchedulerProviderOptions
): AgentSchedulerProvider & {
  ...
  let useNativeTools = !localProvider && agentTools.length > 0;
  let temperature = 0.3;
  ...
  return {
    ...
    async completeModelStep(input): Promise<SchedulerModelStep> {
      ...
      const resolved = resolveAssistantToolCalls({
        ...
        reasoning_content: (message as { reasoning_content?: string | null })
          .reasoning_content,
        tool_calls: message.tool_calls,
        function_call: (
          message as { function_call?: { name?: string; arguments?: string } }
        ).function_call,
      });

      const truncatedOutput =
        choice?.finish_reason === "length" && resolved.truncatedInlineTool;

      const toolCalls: AgentToolCallRequest[] = sortToolsForExecution(
        resolved.toolCalls.filter((item) => item.type === "function")
      )
      ...
      return {
        ...
        truncatedPath: truncatedOutput
          ? peekTruncatedWriteFilePath(
              [message.content, (message as { reasoning_content?: string }).reasoning_content]
