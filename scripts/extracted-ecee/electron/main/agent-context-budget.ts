...
/** Грубая оценка размера payload (JSON в API). */
...
function messageTextLength(message: OpenAiMessage): number {
  ...
  if (Array.isArray(content)) {
    return content
      .map((part) => {
        if (part.type === "text" && "text" in part) return part.text.length;
        if (part.type === "image_url") return 400;
        return 0;
      })
...
/** Урезает блок снимка workspace в system prompt. */
export function trimWorkspaceSnapshotInSystem(
  systemContent: string,
  maxSnapshotChars = 3_500
): string {
  const start = systemContent.indexOf("--- Состояние открытой рабочей папки ---");
  if (start < 0) return systemContent;
  const end = systemContent.indexOf("--- конец снимка ---", start);
  if (end < 0) return systemContent;

  const blockEnd = end + "--- конец снимка ---".length;
  const block = systemContent.slice(start, blockEnd);
  if (block.length <= maxSnapshotChars) return systemContent;

  const trimmed = truncateText(
    block,
    maxSnapshotChars,
    "(снимок папки сокращён — list_directory при нужде)"
  );
  return systemContent.slice(0, start) + trimmed + systemContent.slice(blockEnd);
}

/**
 * Весовая обрезка контекста (порт Void convertToLLMMessageService).
 * Старые assistant/tool сообщения режутся раньше; system и последние user — почти не трогаем.
 */
export function weightBasedTrimMessages(
  ...
  if (trimmableIndexes.length === 0) return false;
  ...
  const weight = (index: number): number => {
    ...
    if (alreadyTrimmed.has(index)) multiplier = 0;
  ...
  const minKeep = Math.min(charBudget, 5_000);
...
function truncateToolMessages(
  ...
  const trimSet = new Set(
    toolIndexes.slice(0, Math.max(0, toolIndexes.length - keepRecent))
  );
  for (const index of toolIndexes) {
    ...
    if (content.length > maxChars) {
      message.content = truncateText(
        content,
        maxChars,
        "(сокращено для лимита контекста)"
      );
    }
  }
}

function dropMiddleAssistantRounds(
  messages: OpenAiMessage[],
  keepTail: number
): void {
  ...
  for (const index of dropFrom) {
    ...
    for (const call of message.tool_calls ?? []) {
      if (call.id) dropToolIds.add(call.id);
    }
  }
  ...
  if (system?.role === "system" && typeof system.content === "string") {
    const note =
      "[Скрыто ранние шаги агента — контекст сокращён. read_file / list_directory при нужде.]\n\n";
...
/**
 * Ужимает messages перед вызовом API (как Void + fallback для Mistral 400).
 */
export function compactMessagesForApi(
  ...
  const system = messages[0];
  if (system?.role === "system" && typeof system.content === "string") {
    const trimmed = trimWorkspaceSnapshotInSystem(
      system.content,
      aggressive ? 2_000 : 3_500
    );
