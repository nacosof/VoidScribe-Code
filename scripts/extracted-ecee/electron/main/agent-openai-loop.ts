...
import { executeAgentTool, type AgentToolEvent, type OpenAiMessage } from "./agent-tools";
  ...
  await runAgentScheduler({
    ...
    executeTool: async (call) =>
      executeAgentTool({
        ...
        signal,
        onEvent,
        userIntent: getLastUserMessageText(
          schedulerProvider.getMessages() as OpenAiMessage[]
        ),
      }),
    afterToolResult: async ({ result }) => {
      const visionParts =
        supportsVision ? toOpenAiVisionFollowUp(result) : null;
      if (visionParts) {
        schedulerProvider.appendUserContent(visionParts);
      } else if (result.images?.length) {
        schedulerProvider.appendUserContent(
          "[Скриншот страницы не передан в модель — нет vision. Продолжай по тексту capture_page_preview или смени модель на vision.]"
        );
      }
    },
  });
